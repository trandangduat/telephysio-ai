...
export const TrainingScreen: React.FC<TrainingProps> = ({ route, navigation }) => {
    ...
    const handleNextExercise = async (finalDurations?: number[]) => {
        if (!uid || !exercise) return;
        setIsFinishing(true);
        setPaused(true);

        const durationsToSend = finalDurations || [...setDurations, currentSetElapsed];
    ...
    const handleCompleteSet = () => {
        const currentSetDuration = currentSetElapsed;
        const updatedDurations = [...setDurations, currentSetDuration];
        setSetDurations(updatedDurations);
        ...
        if (currentSet < totalSets) {
            ...
            handleNextExercise(updatedDurations);
        }
    };
...
const styles = StyleSheet.create({
    ...
    bottomSheet: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: '#ffffff', borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: spacing.md, paddingBottom: spacing.xl, shadowColor: '#000', shadowOffset: { width: 0, height: -4 }, shadowOpacity: 0.1, shadowRadius: 12, elevation: 20 },
    sheetHeader: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 2 },
    dataRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: spacing.sm },

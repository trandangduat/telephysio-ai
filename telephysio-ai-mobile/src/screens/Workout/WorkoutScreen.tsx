...
/**
 * Thành phần (Component) đại diện cho màn hình kế hoạch tập luyện.
 *
 * @param {Props} props Các thuộc tính của thành phần.
 * @returns {JSX.Element} Giao diện người dùng của màn hình lịch trình.
 */
export const WorkoutScreen: React.FC<Props> = ({ navigation }) => {
    ...
    const [incompleteSessions, setIncompleteSessions] = useState<
        Record<string, IncompleteSession>
    >({});
    ...
    // Thiết lập ngày cho lịch trình hàng tuần
    ...
    const sessionsForSelectedDay = weekSessions.filter((s) => {
        ...
        const sDate = (s.date as any).toDate
            ? (s.date as any).toDate()
    ...
    return (
        <SafeAreaView style={styles.safe} edges={["top"]}>
            ...
            >
                ...
                <View style={styles.header}>
                    <AppText variant="headlineLg" style={styles.title}>
                        {isSelectedToday
                            ...
                            : `${daysOfWeek[selectedDayIndex]}'s Schedule`}
                    </AppText>
                    <AppText variant="bodyMd" style={styles.subtitle}>
                        {isSelectedToday
                            ? t(
                                  "workout.subtitle",
                                  "Complete these exercises to reach your daily goal.",
                              )
                            : "Check your exercises scheduled for this week."}
                    </AppText>
                </View>

                {/* Dải lịch trình hàng tuần theo chiều ngang */}
                    ...
                    <View style={styles.calendarRow}>
                        {daysOfWeek.map((day, index) => {
                            ...
                            const hasActiveAssignment = activeAssignments.some(
                                (a) => {
                                    const sDate = a.scheduledDate
                                        ? (a.scheduledDate as any).toDate
                                            ? (a.scheduledDate as any).toDate()
                                            : new Date(a.scheduledDate as any)
                                        : new Date();
                                    return isSameDate(sDate, dateForDay);
                                },
                            );
                ...
                {/* Nội dung trang động dựa trên lựa chọn */}
                {(() => {
                    const hasSessions = sessionsForSelectedDay.length > 0;
                    const activeForDay = activeAssignments
                        .filter((a) => {
                            const sDate = a.scheduledDate
                                ? (a.scheduledDate as any).toDate
                                    ? (a.scheduledDate as any).toDate()
                        ...
                        .sort((a, b) => {
                            ...
                            const dateB = b.scheduledDate
                                ? (b.scheduledDate as any).toDate
                                    ? (b.scheduledDate as any).toDate()
                                    : new Date(b.scheduledDate as any)
                                : new Date(0);
                    ...
                    const totalForDay =
                        sessionsForSelectedDay.length + activeForDay.length;
                    const completedForDay = sessionsForSelectedDay.length;

                    if (!hasSessions && !hasActive) {
                        return (
                            /* Ngày Trống / Ngày Nghỉ */
                            <View
                                style={
                                    selectedDayIndex > todayIndex
                                        ? styles.futureDayCard
                                        : styles.emptyCard
                                }
                            >
                                <Ionicons
                                ...
                                >
                                    You have no workout scheduled for{" "}
                                    {selectedDayIndex === todayIndex
                                        ? "today"
                                        : "this day"}
                                    .
                    ...
                    return (
                        <>
                            ...
                            >
                                ...
                                >
                                    Completed {completedForDay} out of{" "}
                                    {totalForDay} sessions
                                </AppText>
                            </View>

                            {/* Hiển thị Các Kế hoạch Đang hoạt động cho Ngày được chọn */}
                            {activeForDay.map((assignment) => {
                                const incSession =
                                    incompleteSessions[assignment.id];
                                            ...
                                            {assignment.scheduledDate && (
                                                ...
                                                >
                                                    ...
                                                    >
                                                        {(() => {
                                                            const sd = (
                                                                assignment.scheduledDate as any
                                                            ).toDate
                                                                ? (
                                                                      assignment.scheduledDate as any
                                                                  ).toDate()
                            ...
                            {/* Hiển thị các buổi tập đã hoàn thành cho ngày được chọn */}
                            {hasSessions && (
                                <View
                                    style={{
                                        marginTop: hasActive ? spacing.lg : 0,
                            ...
                            {sessionsForSelectedDay.map((session) => (
                                ...
                                >
                                            ...
                                            {(
                                                ...
                                                return (
                                                    <View
                                                        key={i}
                                                        style={
                                                            styles.mockSummaryRow
                                                        }
                                                    >
                                                        <View
                                                            style={[
                                                                styles.mockSummaryIcon,
                                                                {
                                                                    backgroundColor:
                                                                        (ex.color ||
                                                                            colors.primary) +
                                    ...
                                    >
                                        ...
                                        />
                                        <View style={{ flex: 1 }}>
                                            {(session as any).doctorFeedback ||
                                            (session as any).doctorReview ? (
                                                <AppText
                                                    variant="bodyMd"
                                                    style={{ color: "#334155" }}
                                                >
                                                ...
                                                >
                                                    Doctor has not reviewed this
                                                    session yet. Feedback will

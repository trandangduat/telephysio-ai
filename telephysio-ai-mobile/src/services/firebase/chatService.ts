 ...
 *   - DoctorFeedbackScreen (conversation list: patient, lastMessage, unread, hasFeedback)
 *   - FeedbackScreen → submitFeedback → navigates to DoctorChat
 */
...
// Called by DoctorFeedbackScreen (conversation list)
...
export async function getPatientConversation(patientId: string): Promise<Conversation | null> {
  const snap = await getDocs(
    query(
      collection(db, 'conversations'),
      where('patientId', '==', patientId),
  ...
  return snap.empty ? null : { id: snap.docs[0].id, ...snap.docs[0].data() } as Conversation;
}
...
export async function uploadChatAttachment(
  ...
  const fileRef = ref(storage, `chat/${conversationId}/${Date.now()}_${fileName}`);
